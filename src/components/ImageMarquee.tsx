...
export function ImageMarquee({ images }: { images: MarqueeImage[] }) {
  ...
  return (
    <div className="marquee-pause relative mx-[calc(50%-50vw)] w-screen overflow-hidden">
      ...
      <div className="animate-marquee-rtl flex w-max items-center gap-4 py-2 sm:gap-6">
        {track.map((img, i) => (
          <img
            ...
            loading="lazy"
            draggable={false}
            className="h-36 w-auto shrink-0 rounded-2xl object-cover shadow-xl sm:h-52 lg:h-60"
          />
        ))}
      </div>
    </div>
